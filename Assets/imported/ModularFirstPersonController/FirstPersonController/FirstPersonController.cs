...
public class FirstPersonController : MonoBehaviour
{
    ...
    public void Toggle()
    {
        ...
        if (playerCanMove) playerCanMove = false;
        else playerCanMove = true;

        if (crosshair) crosshair = false;
        else crosshair = true;
        ...
        if (odd)
        {
            originalAngles = new Vector3(playerCamera.transform.localEulerAngles.x, playerCamera.transform.localEulerAngles.y, playerCamera.transform.localEulerAngles.z);
            Vector3 targetAngles = new Vector3(0, playerCamera.transform.localEulerAngles.y, 0);
            odd = false;
            StartCoroutine(SmoothRotateCamera(targetAngles, 0.5f));
        }
        else
        {
            StartCoroutine(SmoothRotateCamera(originalAngles, 0.5f));
            odd = true;
        }

        if (enableCrouch) enableCrouch = false;
        else { enableCrouch = true; }
    }
    ...
    IEnumerator SmoothRotateCamera(Vector3 targetAngles, float duration)
    {
        ...
        {
            playerCamera.transform.localRotation = Quaternion.Slerp(startRotation, targetRotation, (elapsedTime / duration));
