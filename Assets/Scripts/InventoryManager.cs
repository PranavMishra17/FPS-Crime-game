...
public class InventoryManager : MonoBehaviour
{
    ...
    public void AddingtoInv()
    {
        try 
        {
            foreach (InventoryItem item in levelInventory.GetItems())
            {
                ...
                // Find a child GameObject with a specific tag
                Transform parentTransform = newItem.transform; // Replace "gameObject" with the parent GameObject's reference
                ...
                foreach (Transform child in parentTransform)
                {
                    if (child.CompareTag("Name"))
                    {
                        ...
                        childObj.GetComponent<TextMeshProUGUI>().text = item.objectName;
                        break;
                    }
                }
    ...
    public void SaveInventory()
    {
        ...
        foreach (InventoryItem item in levelInventory.GetItems())
        {
            ...
            InventorySaveData.SerializableInventoryItemsz serializedItem = new InventorySaveData.SerializableInventoryItemsz(
                 pngtoSprite(item.spritePath), item.itemInfo, item.spritePath, item.addedtoInv, item.objectName);
        ...
        saveData.playerPosition = fpc.transform.position;
        saveData.playerRotation = fpc.transform.rotation;
        ...
        string savePath = Application.dataPath + "/StreamingAssets/inventorySaveData.json";
        ...
        if (File.Exists(inventorySavePath))
        {
           ...
           // {
                ...
                foreach (InventorySaveData.SerializableInventoryItemsz serializedItem in savedata.serializedItems)
                {
                    ...
                    InventoryItem item = new InventoryItem(pngtoSprite(serializedItem.spritePath), serializedItem.itemInfo, serializedItem.spritePath, serializedItem.objName);
                    item.addedtoInv = true;
                    item.ssAdded = true;
                    Debug.Log("item: " + item.itemInfo);
                    levelInventory.AddItem(item);

                     GameObject obj = GameObject.Find(serializedItem.objName);
                     if (obj != null)
                     {
                     ...
                     {
                         Debug.Log(serializedItem.objName + "Object not found!");
                     }
                }

           // SceneManager.LoadScene(SceneManager.GetActiveScene().buildIndex);
            UpdateInventoryUI();
            fpc.transform.position = savedata.playerPosition;
            fpc.transform.rotation = savedata.playerRotation;

            // Find a GameObject with a specific name
            

            // Find all objects with the specified script
            InventoryItem[] objectsWithScript = FindObjectsOfType<InventoryItem>();

            // Call a function on each object with the script
            foreach (InventoryItem obj in objectsWithScript)
            {
               // obj.LoadBool();
            }
